...
function NavItem({ item, counts, activeView, onClick }) {
  if (!item) return <div style={{ height: 1, background: 'var(--border)', margin: '4px 10px' }} />;
  ...
  return (
    <nav style={styles.nav}>
      ...
      <div style={styles.projectInfo}>
        ...
        <div style={styles.projectName}>{project?.name || '未命名'}</div>
        <div style={styles.projectChapter}>Ch{project?.currentChapter || 1}/65</div>
      </div>
      <div style={styles.navItems}>
        {NAV_ITEMS.map((item, i) => (
          <NavItem key={item?.id || `div-${i}`} item={item} counts={counts} activeView={activeView} onClick={() => handleNav(item)} />
        ))}
      </div>
      <div style={styles.footer}>
        <div style={styles.footerText}>InkVerse v1.0</div>
        <div style={styles.footerSub}>CTRL+Z 撤回 · 空格平移</div>
      </div>
    </nav>
  );
}
...
const styles = {
  nav: { width: 220, minWidth: 220, height: '100vh', background: 'var(--bg-card)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', padding: '20px 14px' },
  brand: { display: 'flex', alignItems: 'center', gap: 10, padding: '0 8px 20px', borderBottom: '1px solid var(--border)', marginBottom: 8 },
  logo: { width: 36, height: 36, borderRadius: 10, background: 'linear-gradient(135deg, #6366F1, #8B5CF6)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontSize: '1.2em', fontWeight: 700 },
  ...
  projectInfo: { display: 'flex', alignItems: 'center', gap: 8, padding: '10px 10px', marginBottom: 4 },
  ...
  navItem: { display: 'flex', alignItems: 'center', gap: 10, padding: '9px 10px', borderRadius: 'var(--radius-sm)', border: 'none', cursor: 'pointer', fontSize: '0.85em', transition: 'all 0.15s', textAlign: 'left', background: 'transparent', color: 'var(--text-secondary)' },
  navIcon: { fontSize: '1em', width: 24, textAlign: 'center' },
  navLabel: { flex: 1 },
  badge: { fontSize: '0.65em', fontWeight: 600, color: 'var(--text-secondary)', background: 'var(--accent-hover)', padding: '1px 6px', borderRadius: 8 },
  footer: { marginTop: 'auto', padding: '12px 10px', borderTop: '1px solid var(--border)' },
